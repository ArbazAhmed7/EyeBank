...
{
    public class GothGlassDispenseResidentRepo
    {
        DBHelper.DBHelper db;
        public GothGlassDispenseResidentRepo()
        {
        ...
        public DisplayGothGlassDispenseResidentModel GetResidentLastHistory(DataTable dt)
        {
        ...
        public GothGlassDispenseResidentModel GlassDispenseModel(DataTable dt)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    if (!row["OptometristGothResidentId"].Equals(DBNull.Value))
                        model.OptometristGothResidentId = Convert.ToInt32(row["OptometristGothResidentId"].Equals(DBNull.Value));

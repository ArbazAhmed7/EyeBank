...
{
    [Area("Localities")]
    [Route("Localities/OptometristResident")]
    public class OptometristResidentController : Microsoft.AspNetCore.Mvc.Controller
    {
        OptometristResidentRepo repo;
        [Utilities.ViewRightsAuthorizationFilter(FormId = "LocalityOptometristResident")]
        ...
        public IActionResult Add(int ResidentAutoId = 0)
        {
            ...
            return View("~/Areas/Localities/Views/OptometristResident/Index.cshtml");
        }
        ...
        public JsonResult GetOptometristResidentById(string OptometristResidentId)
        {
            ...
            repo = new OptometristResidentRepo();
            List<SqlParameter> sql = SqlPara("GetById");
            sql.Add(new SqlParameter("@OptometristResidentId", @OptometristResidentId));
            dt = repo.GetForModelFromDB("[Sp_OptometristResident]", sql);
        ...
        public async Task<JsonResult> SaveUpdate(OptometristGothResidentModel Model)
        {
            ...
            if (Model.OptometristResidentId > 0)
                parameters = SqlPara("Update");
            else
                parameters = SqlPara("Save");

            parameters = repo.SetModel(parameters, Model);
            dt = await repo.DbFunction("[Sp_OptometristResident]", parameters);

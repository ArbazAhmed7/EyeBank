...
$('#chkDisplay').on('change', function () {
    if ($('#chkDisplay').is(':checked')) {
        ...
        if ($('#txtGothAutoId').val() != "" && $('#txtGothName').val() != "")
...
$('#btnSave').click(function () {
    ...
    //var response = _ConfirmAjaxRequest(title, content, "/Goths/GothsResidentEnrollment/SaveUpdate", Model, 'json', 'post', null, 'tblDetail');
    ...
    $.confirm({
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: "/Goths/GothsResidentEnrollment/SaveUpdate",
                    data: Model,
                    method: 'post',
                    dataType: 'json',
                    success: function (result) {
...
function onActionButtonClicked(actionName, data) {
    ...
    switch (actionName.toLowerCase()) {
        case 'edit':
            window.location = "/Goths/GothsResidentEnrollment/Add/" + data[0];
            break;
        case 'delete':
            _ConfirmAjaxRequest('Delete', 'Do you want to Delete?', "/Goths/GothsResidentEnrollment/Delete/" + data[0], null, 'json', 'Delete', '/Goths/GothsResidentEnrollment/GetGothList');
            break;
    ...
    $("#txtGothName").autocomplete({
        ...
        select: function (event, ui) {
            ...
            setTimeout(function () {
                ...
                // GetGothModal($('#txtGothAutoId').val());
...
function GetAutoCompleteResident() {
    ...
    if ($('#txtGothAutoId').val() > 0) {
        $.ajax({
            ...
            url: '/DropDownLookUp/Help/GetResidentsForGoth',
            data: GeResidentText(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
...
function RemoveById(id, obj) {
    //_ConfirmAjaxRequest('Delete', 'Do you want to Delete?', "/Goths/GothsResidentEnrollment/DeleteByImage/" + id, null, 'json', 'Delete', null, null, 'Goths/GothsResidentEnrollment/Add/' + RefreshId);
    $.confirm({
        title: 'Delete',
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: '/Goths/GothsResidentEnrollment/DeleteByImage/' + id,
                    method: 'post',
                    dataType: 'json',
                    success: function (result) {
                        //console.log(result);
...
function validate() {
    ...
    if ($('#txtGothCode').val() == '') {
        returnVal = false;
        AddVAlidationToControl("txtGothCode", "show", "Required", "top")
        $(window).scrollTop(0);
    }
    else {
...
$('#btnBack').click(function () {
    window.location = "/Goths/GothsResidentEnrollment/list";
});
    ...
    if ($('#txtResidentAutoId').val() != '' && $('#txtResidentAutoId').val() != 'select' && $('#txtResidentAutoId').val() != '0') {
        $.confirm({
            ...
            buttons: {
                confirm: function () {
                    $.ajax({
                        url: '/Goths/GothsResidentEnrollment/Delete/' + $('#txtResidentAutoId').val(),
                        method: 'post',
                        dataType: 'json',
                        success: function (result) {
                            //console.log(result);

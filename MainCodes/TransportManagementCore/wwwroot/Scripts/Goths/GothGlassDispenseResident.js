var populateDtt = '/Goths/AutoRefTestResident/GetHistoryById/' + $('#txtResidentAutoId').val() + '; Auto Refraction Test History';
...
function GetModal() {
    $.ajax({
        url: '/Goths/GlassDispenseResident/GetLastOptoById',
        method: 'get',
...
function SetValues(object) {
    ...
    $('#txtOptometristGothResidentId').val(object.optometristGothResidentId)
}
...
function GetAutoCompleteGoth() {
    ...
    Goths = [];
    $.ajax({
        url: '/DropDownLookUp/Help/GetGothsForGlassDispense',
        data: GetGothText(),
        method: 'get',
        dataType: 'json',
        success: function (result) {
...
function GetAutoCompleteResident() {
    ...
    if ($('#txtGothAutoId').val() > 0) {
        $.ajax({
            url: '/DropDownLookUp/Help/GetGothResidentsForGlassDispense',
            data: GeResidentText(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
...
function ResetAutoCompleteResident() {
    ...
    $('#txtGlassDispenseResidentId').val('');
}
function ResetResidentValue() {
    ...
    $('#txtLastRightSpherical').val('');
    $('#txtLastIPD').val('');
    $('#txtOptometristGothResidentId').val('')
    $('#txtAge').val('');
    $('#txtGender').val(''); 
    $('#chkWearGlasses').prop('checked', false);
    $('#chkDistance').prop('checked', false);
...
function GetDates() {
    if (($('#chkDisplay').is(':checked')) || $('#chkEdit').is(':checked')) {
        $.ajax({
            url: '/Goths/GlassDispenseResident/GetDatesofGlassDispenseResident/' + $('#txtResidentAutoId').val(),
            method: 'get',
            type: 'json',
            async: false,
            success: function (result) {
...
$('#ddlGetGlassDispenseId').change(function () {
    $('#txtGlassDispenseResidentId').val($('#ddlGetGlassDispenseId').val());
    if ($('#ddlGetGlassDispenseId').val() > 0 && $('#txtResidentAutoId').val()>'0') {
        $.ajax({
            url: '/Goths/GlassDispenseResident/GetGlassDispenseById/' + $('#txtGlassDispenseResidentId').val(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
                ResetFields();
...
$('#btnSave').click(function () {
    ...
    if ($('#txtGlassDispenseResidentId').val() || 0 > 0) {
        Title = 'Update';
    }
    $.confirm({
        title: Title,
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: '/Goths/GlassDispenseResident/SaveUpdate',
                    data: Model,
                    method: 'post',
                    dataType: 'json',
                    success: function (result) {
...
function GetModel() {
    var Model = { 
        GlassDispenseResidentId: $('#txtGlassDispenseResidentId').val(),
        GlassDispenseResidentTransDate: GetVisitDate(),
        ResidentAutoId: $('#txtResidentAutoId').val(),
        ...
        OptometristGothResidentId: $('#txtOptometristGothResidentId').val()
    }
    return Model;
}
...
function SetModel(data) {
    ...
    $('#txtGlassDispenseResidentId').val(data["glassDispenseResidentId"]);
...
$('#btnDelete').click(function () {
    if ($('#txtGlassDispenseResidentId').val() != '' && $('#txtGlassDispenseResidentId').val() != '0') {
        $.confirm({
            title: 'Delete',
            ...
            buttons: {
                confirm: function () {
                    $.ajax({
                        url: '/Goths/GlassDispenseResident/DeleteById/' + $('#txtGlassDispenseResidentId').val(),
                        method: 'post',
                        dataType: 'json',
                        success: function (result) {
                            //console.log(result);

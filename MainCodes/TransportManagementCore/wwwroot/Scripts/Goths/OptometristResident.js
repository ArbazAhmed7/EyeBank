...
$('#chkNew').on('change', function () {
    if ($('#chkNew').is(':checked')) {
        ...
        $('#txtGothOptometristResidentId').val('')
        $('#txtAutoRefResidentId').val('')
        $('#txtformMode').val('New');
        $('#btnSave').text('Save');
        Clear();
        if ($('#txtGothCode').val() != "" && $('#autocomplete-input-Goth').val() != "")
...
function ResetGothValue() {
    document.getElementById("autocomplete-input-Goth").value = '';
    document.getElementById("txtGothCode").value = '';
    document.getElementById("txtGothId").value = '';
}
function GetModal(Id) {
    $.ajax({
        url: '/Goths/GothAutoRefTestResident/GetAutoRefByResidentId/' + Id,
        method: 'get',
        dataType: 'json',
...
function GetDates() {
    if (($('#chkDisplay').is(':checked')) || $('#chkEdit').is(':checked')) {
        $.ajax({
            url: '/Goths/GothOptometristResident/GetDatesofResident/' + $('#txtResidentAutoId').val(),
            method: 'get',
            type: 'json',
            success: function (result) {
                console.log(result);
                $('#ddlGetGothOptometristResidentById').empty();
                $("#ddlGetGothOptometristResidentById").append($("<option></option>").val('').html(' Date'));
                $('#txtGothOptometristResidentId').val('');
                $.each(result, function (data, value) {
                    $("#ddlGetGothOptometristResidentById").append($("<option></option>").val(value.code).html(value.text));
                })
                $('#divDisplay').show();
                $('#ddlGetGothOptometristResidentById').show()
                $('#ddlGetGothOptometristResidentById').focus();
            }
        })
        setTimeout(function () {
            var total_tems = $('#ddlGetGothOptometristResidentById').find('option').length;
            if (total_tems == 2) {
                $('#ddlGetGothOptometristResidentById option:nth-child(2)').attr('selected', true);
                $('#ddlGetGothOptometristResidentById').change()
            }
        ...
        $('#divDisplay').hide();
        $('#ddlGetGothOptometristResidentById').hide()
    }
    if ($('#chkDisplay').is(':checked')) {
        $('#btnSave').hide();
    }
    ...
    if ($('#chkDisplay').is(':checked') || $('#chkEdit').is(':checked'))
        if ($('#autocomplete-input-Goth').val() == "" || $('#autocomplete-input-Resident').val() == "") {
            $('#divDisplay').hide();
            $('#ddlGetGothOptometristResidentById').hide();
        }
}
...
$('#ddlGetGothOptometristResidentById').change(function () {
    $('#txtGothOptometristResidentId').val($('#ddlGetGothOptometristResidentById').val());
    if ($('#txtGothOptometristResidentId').val() > 0) {
        $.ajax({
            url: '/Goths/GothOptometristResident/GetOptometristResidentById/' + $('#txtGothOptometristResidentId').val(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
                //ResetFields();
...
function SetModel(SetValues) {
    ...
    Clear();
    $('#txtGothOptometristResidentId').val(SetValues.optometristGothResidentId);
...
function SetValues(object) {
    ...
    //$('#ddlGoth').val(object.GothCode).trigger('change');
    //$('#txtGothCode').val(object.GothCode)
    //$('#txtGothAutoId').val(object.GothAutoId);
...
$("#autocomplete-input-Goth").keydown(function (e) {
...
function GetGothText() {
    ...
    var Model = {
        New: Type,
        SearchText: $("#autocomplete-input-Goth").val()
    }
    return Model;
}
...
function GetAutoCompleteComapny() {
    ...
    $.ajax({
        //url: '/DropDownLookUp/Help/GetGothsForGothOptometristResident/' + Type+'/'+$("#autocomplete-input-Goth").val(),
        url: '/DropDownLookUp/Help/GetGothsForOptometristResident',
        data: GetGothText(),
        method: 'get',
        dataType: 'json',
        success: function (result) {
    ...
    $("#autocomplete-input-Goth").autocomplete({
        source: function (request, response) {
            var term = request.term.toLowerCase();
            // Filter the item list based on the search term
            var filteredList = $.grep(Goths, function (item) {
                //console.log(Goths);
        ...
        select: function (event, ui) {
            ...
            setTimeout(function () {
                document.getElementById("autocomplete-input-Goth").value = selectedName;
...
function GetAutoCompleteResident() {
    ...
    if ($('#txtGothId').val() > 0) {
        ...
        $.ajax({
            //url: '/DropDownLookUp/Help/GetResidentsForOptometrist/' + $('#txtGothId').val() + "/" + Type + "/" + FinalDate,
            url: '/DropDownLookUp/Help/GetGothResidentsForOptometrist',
            data: GeResidentText(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
...
function GetResidentLastHistory() {
    if ($('#txtResidentAutoId').val() || 0 > 0) {
        $.ajax({
            url: '/Goths/GothAutoRefTestResident/GetLastHistoryById/' + $('#txtResidentAutoId').val(),
            method: 'get',
            dataType: 'json',
            async: false,
            success: function (result) {
...
function validateNextAcuity() {
    ...
    if ($('#autocomplete-input-Goth').val() == '') {
        returnVal = false;
        AddVAlidationToControl("autocomplete-input-Goth", "show", "Mandatory", "top")
        $(window).scrollTop(0);
        return returnVal
    }
    else {
        AddVAlidationToControl("autocomplete-input-Goth", "hide", "", "bottom")
    }

    if ($('#txtGothCode').val() == '') {
        returnVal = false;
        AddVAlidationToControl("txtGothCode", "show", "Mandatory", "top")
        $(window).scrollTop(0);
    }
    else {
        AddVAlidationToControl("txtGothCode", "hide", "", "bottom")
    }
...
function GetModel() {
    ...
    var Model = {
        OptometristGothResidentId: $('#txtGothOptometristResidentId').val(),
        AutoRefResidentId: $('#txtAutoRefResidentId').val(),
        OptometristGothResidentTransDate: GetVisitDate(),
        ResidentAutoId: $('#txtResidentAutoId').val(),
        GothAutoId: $('#txtGothAutoId').val(),
        HasChiefComplain:0,
...
$('#btnSave').click(function () {
    ...
    if ($('#txtGothOptometristResidentId').val() || 0 > 0) {
        Title = 'Update';
    }
    $.confirm({
        title: Title,
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: '/Goths/GothOptometristResident/SaveUpdate',
                    data: data,
                    method: 'post',
                    dataType: 'json',
                    success: function (result) {
...
function GetResidentInfo(Id) {
    $.ajax({
        url: '/Goths/GothsResidentEnrollment/GetById/' + Id,
        method: 'get',
        dataType: 'json',
        success: function (result) {
            //console.log(result);
...
$('#btnDelete').click(function () {
    if ($('#ddlGetGothOptometristResidentById').val() != '' && $('#ddlGetGothOptometristResidentById').val() != 'select') {
        $.confirm({
            title: 'Delete',
            ...
            buttons: {
                confirm: function () {
                    $.ajax({
                        url: '/Goths/GothOptometristResident/DeleteById/' + $('#ddlGetGothOptometristResidentById').val(),
                        method: 'post',
                        dataType: 'json',
                        success: function (result) {
                            //console.log(result);

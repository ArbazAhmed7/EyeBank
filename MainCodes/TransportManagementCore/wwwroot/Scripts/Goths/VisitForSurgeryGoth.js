...
function GetAutoCompleteComapny() {
    ...
    companies = [];
    $.ajax({
        url: '/DropDownLookUp/Help/GetGothsForVisitForSurgeryResident',
        data: GetCompanyText(),
        method: 'get',
        dataType: 'json',
    ...
    $("#autocomplete-input-Goth").autocomplete({
        ...
        select: function (event, ui) {
            ...
            setTimeout(function () {
                ...
                document.getElementById("txtGothAutoId").value = selectedId;
                ResetWorkerValue()
                GetAutoCompleteWorker();
...
function GetAutoCompleteWorker() {
    if ($('#txtGothAutoId').val() > 0) {
        $.ajax({
            url: '/DropDownLookUp/Help/GetResidentForVisitForSurgeryGoth',
            data: GeWorkerText(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
...
function GetModal() {
    $.ajax({
        url: '/Goths/VisitForSurgeryResident/GetLastOptoForSurgery',
        method: 'get',
        data: GetWorkerModel(),
        dataType: 'json',
        success: function (result) {
...
function SetValues(object) {
    ...
    $('#txtOptometristResidentId').val(object.optometristGothResidentId)
...
function CreateTableForSavedImages(obj) {
    ...
    for (var i = 0; i < obj.length; i++) {
        if (obj[i]["surgeryGothResidentDocumentsId"] > 0) {
            j = i + 1;
            let
                tr = "<tr data-tranID=" + obj[i]["surgeryGothResidentDocumentsId"] + " data-tranDetailID=" + i + " >";
            tr = tr + "<td scope=row>" + j + "</td>";
            tr = tr + "<td scope=row>" + obj[i]["fileName"] + "</td>"
            tr = tr + "<td scope=row>" + obj[i]["fileType"] + "</td>"
            tr = tr + "<td hidden>" + true + "</td>";
            tr = tr + '<td style="margin-left:-50px;width=150px;"><button onclick="DownloadById(' + obj[i]["surgeryGothResidentDocumentsId"] + ',this)" class="btn btn-primary"><i class="fa fa-save"></i>&nbsp;&nbsp;Download</button></td>';
            tr = tr + '<td style="margin-left:-50px;width=150px;"><button onclick="RemoveById(' + obj[i]["surgeryGothResidentDocumentsId"] + ',this)" class="btn btn-danger"><i class="fa fa-remove"></i>&nbsp;&nbsp;Remove</button></td>';
            tr = tr + "</tr>";
...
function DownloadById(id, obj) {
    window.open(window.location.origin+"/Goths/VisitForSurgeryResident/GetDocumentById/"+id, '_blank');
...
function RemoveById(id, obj) {
    ...
    $.confirm({
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: '/Goths/VisitForSurgeryResident/DeleteById/' + id,
                    method: 'post',
                    dataType: 'json',
                    success: function (result) {
                        //console.log(result);
...
function validate() {
    ...
    if ($('#autocomplete-input-Goth').val() == '') {
        returnVal = false;
        AddVAlidationToControl("autocomplete-input-Goth", "show", "Name is required", "top")
        $(window).scrollTop(0);
    }
    else {
    ...
    if ($('#txtGothCode').val() == '') {
        returnVal = false;
        AddVAlidationToControl("txtGothCode", "show", "Code is required", "top")
        $(window).scrollTop(0);
    }
    else {
...
$('#btnSave').click(function () {
    if (!validate()) { return; }
    formData.append("VisitSurgeryGothResidentId", $('#txtVisitSurgeryResidentId').val());
    formData.append("OptometristGothResidentId", $('#txtOptometristResidentId').val());
    formData.append("ResidentAutoId", $('#txtResidentAutoId').val());
    formData.append("GothAutoId", $('#txtGothAutoId').val());
    ...
    $.confirm({
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: '/Goths/VisitForSurgeryResident/SaveUpdate',
                    data: formData,
                    processData: false,
                    contentType: false,
                    type: "POST",
...
$('#ddlGetHospitalVisitId').change(function () {
    ...
    if ($('#ddlGetHospitalVisitId').val() > 0) {
        $.ajax({
            url: '/Goths/VisitForSurgeryResident/GetSurgeryWorkerById/' + $('#txtVisitSurgeryResidentId').val(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
                ResetFields();
...
function GetDates() {
    if (($('#chkDisplay').is(':checked')) || $('#chkEdit').is(':checked')) {
        $.ajax({
            url: '/Goths/VisitForSurgeryResident/GetDatesofSurgeryWorker/' + $('#txtResidentAutoId').val(),
            method: 'get',
            type: 'json',
            async: false,
            success: function (result) {
...
function SetModel(result) {
    $('#txtVisitSurgeryResidentId').val(result.visitSurgeryGothResidentId);
...
$('#btnDelete').click(function () {
    $.confirm({
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: '/Goths/VisitForSurgeryResident/DeleteSurgeryById/' + $('#txtVisitSurgeryResidentId').val(),
                    method: 'post',
                    dataType: 'json',
                    success: function (result) {
                        //console.log(result);
...
function ResetCompanyValue() {
    ...
    document.getElementById("txtGothAutoId").value = '';
}

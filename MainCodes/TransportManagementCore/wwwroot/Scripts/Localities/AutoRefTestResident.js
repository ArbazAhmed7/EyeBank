...
function GetModal(Id) { 
    $.ajax({
        url: '/Localities/AutoRefTestResident/GetById/' + Id,
        method: 'get',
        dataType: 'json',
...
$('#btnSave').click(function () {
    ...
    $.confirm({
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: '/Localities/AutoRefTestResident/SaveUpdate',
                    data: Model,
                    method: 'post',
                    dataType: 'json',
                    success: function (result) {
...
$('#btnBack').click(function () {
    window.location = "/Localities/AutoRefTestResident/list";
})
...
function onLookupItemSelected(lookupName, data) {
    ...
    switch (lookupName) {
        case 'Localities':
            $('#txtLocalityCode').val(id);
            $('#lookup-Localities-text').val(title);
            break;
        case 'helpDepartment':
...
function GetAutoCompleteLocality() {
    ...
    locality = [];
    $.ajax({
        url: '/DropDownLookUp/Help/GetLocalitiesForAutoRef',
    ...
    $("#autocomplete-input-Locality").autocomplete({
        source: function (request, response) {
            ...
            var filteredList = $.grep(locality, function (item) {
...
function GetAutoCompleteResident() {
    ...
    if ($('#txtLocalityId').val() > 0) {
        ...
        $.ajax({
            ...
            url: '/DropDownLookUp/Help/GetResidentForAutoRef',
            data: GeResidentText(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
...
function GetDates() {
    if (($('#chkDisplay').is(':checked')) || $('#chkEdit').is(':checked')) {
        $.ajax({
            url: '/Localities/AutoRefTestResident/GetDatesofResident/' + $('#txtResidentAutoId').val(),
            method: 'get',
            type: 'json',
            async: false,
            success: function (result) {
...
$('#ddlGetAutoRefId').change(function () {
    ...
    if ($('#txtAutoRefResidentId').val() > 0) {
        $.ajax({
            url: '/Localities/AutoRefTestResident/GetAutoRefById/' + $('#txtAutoRefResidentId').val(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
                ResetFields();
    ...
    if ($('#ddlGetAutoRefId').val() != '' && $('#ddlGetAutoRefId').val() != 'select') {
        $.confirm({
            ...
            buttons: {
                confirm: function () {
                    $.ajax({
                        url: '/Localities/AutoRefTestResident/DeleteById/' + $('#ddlGetAutoRefId').val(),
                        method: 'post',
                        dataType: 'json',
                        success: function (result) {
                            //console.log(result);

...
$('#chkDisplay').on('change', function () {
    if ($('#chkDisplay').is(':checked')) {
        ...
        if ($('#txtPublicSpacesAutoId').val() != "" && $('#txtPublicSpacesName').val() != "")
...
$('#btnSave').click(function () {
    ...
    //var response = _ConfirmAjaxRequest(title, content, "/PublicSpaces/PublicSpacesResidentEnrollment/SaveUpdate", Model, 'json', 'post', null, 'tblDetail');
    ...
    $.confirm({
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: "/PublicSpaces/PublicSpacesResidentEnrollment/SaveUpdate",
                    data: Model,
                    method: 'post',
                    dataType: 'json',
                    success: function (result) {
...
function onActionButtonClicked(actionName, data) {
    ...
    switch (actionName.toLowerCase()) {
        case 'edit':
            window.location = "/PublicSpaces/PublicSpacesResidentEnrollment/Add/" + data[0];
            break;
        case 'delete':
            _ConfirmAjaxRequest('Delete', 'Do you want to Delete?', "/PublicSpaces/PublicSpacesResidentEnrollment/Delete/" + data[0], null, 'json', 'Delete', '/PublicSpaces/PublicSpacesResidentEnrollment/GetPublicSpacesList');
            break;
    ...
    $("#txtPublicSpacesName").autocomplete({
        ...
        select: function (event, ui) {
            ...
            setTimeout(function () {
                ...
                // GetPublicSpacesModal($('#txtPublicSpacesAutoId').val());
...
function GetAutoCompleteResident() {
    ...
    if ($('#txtPublicSpacesAutoId').val() > 0) {
        $.ajax({
            ...
            url: '/DropDownLookUp/Help/GetResidentsForPublicSpaces',
            data: GeResidentText(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
...
function RemoveById(id, obj) {
    //_ConfirmAjaxRequest('Delete', 'Do you want to Delete?', "/PublicSpaces/PublicSpacesResidentEnrollment/DeleteByImage/" + id, null, 'json', 'Delete', null, null, 'PublicSpaces/PublicSpacesResidentEnrollment/Add/' + RefreshId);
    $.confirm({
        title: 'Delete',
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: '/PublicSpaces/PublicSpacesResidentEnrollment/DeleteByImage/' + id,
                    method: 'post',
                    dataType: 'json',
                    success: function (result) {
                        //console.log(result);
...
function validate() {
    ...
    if ($('#txtPublicSpacesCode').val() == '') {
        returnVal = false;
        AddVAlidationToControl("txtPublicSpacesCode", "show", "Required", "top")
        $(window).scrollTop(0);
    }
    else {
...
$('#btnBack').click(function () {
    window.location = "/PublicSpaces/PublicSpacesResidentEnrollment/list";
});
    ...
    if ($('#txtResidentAutoId').val() != '' && $('#txtResidentAutoId').val() != 'select' && $('#txtResidentAutoId').val() != '0') {
        $.confirm({
            ...
            buttons: {
                confirm: function () {
                    $.ajax({
                        url: '/PublicSpaces/PublicSpacesResidentEnrollment/Delete/' + $('#txtResidentAutoId').val(),
                        method: 'post',
                        dataType: 'json',
                        success: function (result) {
                            //console.log(result);

...
function GetModal(Id) {
    $.ajax({
        url: '/Factory/AutoRefTestWorker/GetAutoRefByWorkerId/' + Id,
        method: 'get',
        dataType: 'json',
...
function GetDates() {
    if (($('#chkDisplay').is(':checked')) || $('#chkEdit').is(':checked')) {
        $.ajax({
            url: '/Factory/OptometristWokrer/GetDatesofWorker/' + $('#txtWorkerAutoId').val(),
            method: 'get',
            type: 'json',
            success: function (result) {
                ////console.log(result);
                $('#ddlGetOptometristWokrerById').empty();
                $("#ddlGetOptometristWokrerById").append($("<option></option>").val('').html(' Date'));
                $('#txtOptometristWorkerId').val('');
                $.each(result, function (data, value) {
                    $("#ddlGetOptometristWokrerById").append($("<option></option>").val(value.code).html(value.text));
                })
                $('#divDisplay').show();
                $('#ddlGetOptometristWokrerById').show()
                $('#ddlGetOptometristWokrerById').focus();
            }
        })
        setTimeout(function () {
            var total_tems = $('#ddlGetOptometristWokrerById').find('option').length;
            if (total_tems == 2) {
                $('#ddlGetOptometristWokrerById option:nth-child(2)').attr('selected', true);
                $('#ddlGetOptometristWokrerById').change()
            }
        ...
        $('#divDisplay').hide();
        $('#ddlGetOptometristWokrerById').hide()
    }
    if ($('#chkDisplay').is(':checked')) {
        $('#btnSave').hide();
    }
...
$('#ddlGetOptometristWokrerById').change(function () {
    $('#txtOptometristWorkerId').val($('#ddlGetOptometristWokrerById').val());
    if ($('#txtOptometristWorkerId').val() > 0) {
        $.ajax({
            url: '/Factory/OptometristWokrer/GetOptometristWokrerById/' + $('#txtOptometristWorkerId').val(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
                //ResetFields();
...
function SetModel(SetValues) {
    ...
    Clear();
    $('#txtOptometristWorkerId').val(SetValues.optometristWorkerId);
    $('#txtAutoRefWorkerId').val(SetValues.autoRefWorkerId);
...
function SetValues(object) {
    ...
    $('#txtAutoRefWorkerId').val(object.autoRefWorkerId)
...
function GetAutoCompleteComapny() {
    ...
    $.ajax({
        //url: '/DropDownLookUp/Help/GetCompaniesForOptometristWorker/' + Type+'/'+$("#autocomplete-input-company").val(),
        url: '/DropDownLookUp/Help/GetCompaniesForOptometristWorker',
        data: GetCompanyText(),
        method: 'get',
        dataType: 'json',
        success: function (result) {
...
function GetAutoCompleteWorker() {
    ...
    if ($('#txtCompanyId').val() > 0) {
        ...
        $.ajax({
            //url: '/DropDownLookUp/Help/GetWorkersForOptometrist/' + $('#txtCompanyId').val() + "/" + Type + "/" + FinalDate,
            url: '/DropDownLookUp/Help/GetWorkersForOptometrist',
            data: GeWorkerText(),
            method: 'get',
            dataType: 'json',
            success: function (result) {
        ...
        $("#autocomplete-input-worker").autocomplete({
            ...
            select: function (event, ui) {
                ...
                setTimeout(function () {
                    ...
                    GetDates();
                    GetWorkerInfo($("#txtWorkerAutoId").val())
...
function NextFromAcuity() {
    ...
    // 1. If VA 6/6 and N6 worker go to Diagnostic Page.
    if (
    ...
    //  1. IF VA is less than 6/6 worker, it will move to Subjective Refraction and then on Diagnosis
...
function GetModel() {
    ...
    var Model = {
        ...
        OptometristWorkerTransDate: GetVisitDate(),
...
$('#btnSave').click(function () {
    ...
    if ($('#txtOptometristWorkerId').val() || 0 > 0) {
        Title = 'Update';
    }
    $.confirm({
        title: Title,
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    url: '/Factory/OptometristWokrer/SaveUpdate',
                    data: data,
                    method: 'post',
                    dataType: 'json',
                    success: function (result) {
                        ...
                        if (result.toLowerCase().includes('successfully')) {
                            $.confirm({
                                title: result,
                                content: "Do you want to add other worker's Optometrist Test also?",
                                buttons: {
                                    Yes: function () {
                                        ResetWorkerValue();
                                        //ResetFields();
...
function GetWorkerInfo(Id) {
    $.ajax({
        ...
        dataType: 'json',
        success: function (result) {
           // //console.log("rafas",result);
           SetValuesWorker(result);
        }
    })
}


function SetValuesWorker(object) {
    $('#txtAge').val(object.age);
    setTimeout(function () {
...
$('#btnDelete').click(function () {
    if ($('#ddlGetOptometristWokrerById').val() != '' && $('#ddlGetOptometristWokrerById').val() != 'select') {
        $.confirm({
            title: 'Delete',
            ...
            buttons: {
                confirm: function () {
                    $.ajax({
                        url: '/Factory/OptometristWokrer/DeleteById/' + $('#ddlGetOptometristWokrerById').val(),
                        method: 'post',
                        dataType: 'json',
                        success: function (result) {
                            ////console.log(result);

...
{
    public partial class rptforGlasses_DRRE : System.Web.UI.Page
    {
        secoffEntities dx = new secoffEntities();
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Utilities.CanView(Utilities.GetLoginUserID(), "rptforGlasses_DRRE"))
            {
        ...
        protected void btnView_Click(object sender, EventArgs e)
        {
            try
            {
                if (ValidateInput())
                {
                    ...
                    {
                        string jsReport = "<script type='text/javascript'>var newHeight = screen.height - 80;window.open('ReportViewerRDLC.aspx?vReportName=rptRefractedErrorReport&FromDate=" + strFromDate + "&ToDate=" + strToDate + "&SchoolAutoId=" + hfSchoolIDPKID.Value + "&ReportType=" + rdoReportType.SelectedValue + "','','height='+newHeight+',width=1600,menubar=no,status=yes,location=no,top=1,left=1,scrollbars=yes,resizable=yes')</script>";
                    ...
                    if (dtSchool != null)
                    {
                        if (dtSchool.Rows.Count > 0)
                        {
                            ...
                            using (ExcelPackage excel = new ExcelPackage())
                            {
                                ...
                                workSheet = excel.Workbook.Worksheets.Add("RefractedErrorsDetailReport");
                                ...
                                if (dtMainReport_FilteredTable.Rows.Count > 0)
                                {
                                    ...
                                    workSheet.Cells[FirstRow, 10].Value = "Glass Dispensing Date";
                                    ...
                                    foreach (DataRow dr in dtMainReport_FilteredTable.Rows)
                                    {
                                        ...
                                        foreach (DataColumn dc in dtMainReport_FilteredTable.Columns)
                                        {
                                            ...
                                            if (FirstColumn == 1 || FirstColumn == 2 || FirstColumn == 3 || FirstColumn == 4 || FirstColumn == 5 || FirstColumn == 6 || FirstColumn == 7 || FirstColumn == 8 || FirstColumn == 11)
                                            {
                                                workSheet.Cells[FirstRow, FirstColumn, FirstRow, FirstColumn].Value = dr[dc].ToString();
                                ...
                                Response.AddHeader("content-disposition", "attachment;  filename=RefractedErrorsDetailReport.xlsx");

...
{
    public partial class rptforGlasses_PCRS : System.Web.UI.Page
    {
        secoffEntities dx = new secoffEntities();
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Utilities.CanView(Utilities.GetLoginUserID(), "rptforGlasses_PCRS"))
            {
        ...
        protected void btnView_Click(object sender, EventArgs e)
        {
            try
            {
                if (ValidateInput())
                {
                    ...
                    {
                        string jsReport = "<script type='text/javascript'>var newHeight = screen.height - 80;window.open('ReportViewerRDLC.aspx?vReportName=rptCycloRefractionGlassesnotProvided&FromDate=" + strFromDate + "&ToDate=" + strToDate + "&SchoolAutoId=" + hfSchoolIDPKID.Value + "&ReportType=" + rdoReportType.SelectedValue + "','','height='+newHeight+',width=1600,menubar=no,status=yes,location=no,top=1,left=1,scrollbars=yes,resizable=yes')</script>";
                    ...
                    if (dtSchool != null)
                    {
                        if (dtSchool.Rows.Count > 0)
                        {
                            ...
                            using (ExcelPackage excel = new ExcelPackage())
                            {
                                ...
                                workSheet = excel.Workbook.Worksheets.Add("PendingCycloplagicRefraction");
                                ...
                                Utilities.SetTimeStamp(workSheet, "Pending Cycloplagic Refraction for Students");
                                ...
                                if (dtMainReport_FilteredTable.Rows.Count > 0)
                                {
                                    ...
                                    workSheet.Cells[FirstRow, 3].Value = "Class";
                                    workSheet.Cells[FirstRow, 4].Value = "Section";
                                    workSheet.Cells[FirstRow, 5].Value = "Father Name";
                                    workSheet.Cells[FirstRow, 6].Value = "Father Cell";
                                    workSheet.Cells[FirstRow, 7].Value = "Mother Name";
                                    workSheet.Cells[FirstRow, 8].Value = "Mother Cell";
                                    workSheet.Cells[FirstRow, 9].Value = "Opto Date";
                                    workSheet.Cells[FirstRow, 10].Value = "School Name";
                                ...
                                Response.AddHeader("content-disposition", "attachment;  filename=PendingCycloplagicRefraction.xlsx");
        ...
        protected void btnLookupSchool_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable data = (from a in dx.sp_GetLookupData_School_CycloReport()
